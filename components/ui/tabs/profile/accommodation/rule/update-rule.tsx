...
import { RulePatchSchema, RulePatch, RuleResponse } from "@/types/zod/rule";
import { Edit, Save, X } from "lucide-react-native";

interface UpdateRuleFormProps {
  ruleData: RuleResponse;
  onUpdate: (id: string, data: RulePatch) => Promise<void>;
}

export function UpdateRuleModal({ ruleData, onUpdate }: UpdateRuleFormProps) {
  ...
  const {
    ...
    resolver: zodResolver(RulePatchSchema),
    defaultValues: {
  ...
  const onSubmit = async (data: RulePatch) => {
    setIsLoading(true);
    try {
      await onUpdate(ruleData._id, data);
      setOpen(false);
    } catch (error: any) {
      Alert.alert(
        "Erreur",
  ...
  return (
    <>
      ...
      >
        ...
        <Text className="text-gray-800 ml-2 font-medium">Modifier</Text>
      </TouchableOpacity>
          ...
          <View className="bg-gray-50 rounded-t-3xl max-h-[90%]">
            <ScrollView className="px-5 pt-6 pb-10">
              ...
              <Text className="text-gray-600 mb-6">
                Apportez des modifications à votre règle ici. Cliquez sur
